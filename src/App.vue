<template>
    <div class="left" ref="aside">
        <div class="top">
            <button @click="closeAside" title="关闭边栏" class="asideButton"><svg width="24" height="24" viewBox="0 0 24 24"
                    fill="none" xmlns="http://www.w3.org/2000/svg" class="icon-xl-heavy">
            ...
            <button @click="createNewChat" title="新建对话" class="asideButton"><svg width="24" height="24"
                    viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" class="icon-xl-heavy">
                    <path
                ...
                <ul>
                    <li v-for="prompt in prompts" :key="prompt.id">
                        ...
                        <button @click="morePromptButtonIsShowed = prompt.id" type="button" id="radix-:rd8:"
                            aria-haspopup="menu" aria-expanded="false" data-state="closed"><svg width="24" height="24"
                        ...
                        <div class="moreButton" v-if="morePromptButtonIsShowed == prompt.id">
                            <ul>
                                <li @click="renamePrompt(prompt.id)">重命名</li>
                                <li @click="updatePrompt(prompt.id)">更改预设</li>
                            </ul>
                        </div>
                    </li>
                </ul>
            ...
            <div class="bottom">
                <ul>
                    <li v-for="chat in chatHistory" :key="chat.id">
                        <div class="chatName" @click="getChatContent(chat.id)">{{ chat.name }}</div>
                        <button @click="moreButtonIsShowed = chat.id" type="button" id="radix-:rd8:"
                            aria-haspopup="menu" aria-expanded="false" data-state="closed"><svg width="24" height="24"
                                viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="icon-md">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M3 12C3 10.8954 3.89543 10 5 10C6.10457 10 7 10.8954 7 12C7 13.1046 6.10457 14 5 14C3.89543 14 3 13.1046 3 12ZM10 12C10 10.8954 10.8954 10 12 10C13.1046 10 14 10.8954 14 12C14 13.1046 13.1046 14 12 14C10.8954 14 10 13.1046 10 12ZM17 12C17 10.8954 17.8954 10 19 10C20.1046 10 21 10.8954 21 12C21 13.1046 20.1046 14 19 14C17.8954 14 17 13.1046 17 12Z"
                        ...
                        <div class="moreButton" v-if="moreButtonIsShowed == chat.id">
                            <ul>
    ...
    <div class="right" ref="mainPage">
        ...
        <div class="msgInput">
            <input placeholder="向AI发送信息，回车发送" type="text" ref="inputMsg" @keydown.enter="sendMsgToService">
            <button @click="sendMsgToService" class="sendButton"><svg width="32" height="32" viewBox="0 0 32 32"
...
const prompts = ref([
    ...
    },
    {
        id: 1,
        name: '英语翻译',
        content: "我希望你能充当英语翻译、拼写纠正者和改进者。我将用任何语言与你交谈，你将检测语言，翻译它，并在我的文本的更正和改进版本中用英语回答。我希望你用更漂亮、更优雅、更高级的英语单词和句子来取代我的简化 A0 级单词和句子。保持意思不变，但让它们更有文学性。我希望你只回答更正，改进，而不是其他，不要写解释。"
    },
    {
        id: 2,
        name: '预设2',
...
const ws = new WebSocket('wss://watering.top/ws')
ws.onmessage = (event) => {
...
onMounted(() => {
    chatHistory.value = localStorage.getItem('chatHistory') ? JSON.parse(localStorage.getItem('chatHistory')) : []
    id.value = localStorage.getItem('id') ? JSON.parse(localStorage.getItem('id')) : 0
    prompts.value = localStorage.getItem('prompts') ? JSON.parse(localStorage.getItem('prompts')) : prompts.value
    createNewChat()
})
