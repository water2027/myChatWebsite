<template>
    <div class="left" ref="aside">
        <div class="top">
            ...
            <button @click="createNewChat" title="新建对话" class="asideButton">
                <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
        ...
        <div class="selection">
            <div class="prompt">
                <ul>
                    <li>
                        <div class="chatName" @click="isStream = !isStream">
                            {{ isStream ? '目前流式' : '目前非流式' }}
                        </div>
                    </li>
                    <li>
                        ...
                        <button
                            @click="
                                morePromptButtonIsShowed =
                                    morePromptButtonIsShowed == prompt.id ? -1 : prompt.id
                            "
                            type="button"
                            id="radix-:rd8:"
                            aria-haspopup="menu"
    ...
    <div class="right" ref="mainPage">
        ...
        <div class="msgInput">
            ...
            />
            <button @click="sendMsgToService" class="sendButton">
                <svg
                    width="32"
                    height="32"
                    viewBox="0 0 32 32"
...
watch(isStream, (newVal) => {
    localStorage.setItem('isStream', JSON.stringify(newVal))
    if (newVal) {
        initws()
    } else {
        ws.value.close()
...
onMounted(() => {
    ...
    models.value = localStorage.getItem('models')
        ...
        : models.value
    createNewChat()
    isStream.value = localStorage.getItem('isStream')
        ? JSON.parse(localStorage.getItem('isStream'))
        : false
    if (isStream.value) {
        initws()
    }
